...
def convert_tournament_format(tournament_games):
    """
    Convert tournament_games DataFrame to a format where each team has its own row for each day/round.
    
    Parameters:
    tournament_games: DataFrame with columns team1, team2, seed_team1, seed_team2, odds_team1, team1_win, tournament_round, day
    
    Returns:
    List of DataFrames, one for each round/day combination with columns:
    teamname, teamseed, playable, odds, match no
    """
    import pandas as pd
    import numpy as np
    
    # Convert tournament_games to DataFrame if it's a list
    if isinstance(tournament_games, list):
        tournament_games = pd.DataFrame(tournament_games)
    
    # Find all unique round/day combinations
    round_days = tournament_games[['tournament_round', 'day']].drop_duplicates().values
    
    # Create list to store DataFrames for each round/day
    all_day_dfs = []
    
    for tournament_round, day in round_days:
        # Get games for this round and day
        day_games = tournament_games[(tournament_games['tournament_round'] == tournament_round) & 
                                   (tournament_games['day'] == day)]
        
        # Create a DataFrame with all 64 teams
        all_teams = pd.DataFrame({
            'teamname': [str(i) for i in range(1, 65)],  # Teams named 1-64
            'teamseed': [(i-1) % 16 + 1 for i in range(1, 65)],  # Seeds 1-16 repeated for each region
            'playable': 0,  # Default to not playing
            'odds': 0.0,  # Default odds
            'match no': 0  # Default match number
        })
        
        # Create a match counter for this day
        match_counter = 1
        
        # Update team info for teams playing in this round/day
        for _, game in day_games.iterrows():
            team1 = game['team1']
            team2 = game['team2']
            
            # Find indices for these teams
            team1_idx = all_teams[all_teams['teamname'] == team1].index
            team2_idx = all_teams[all_teams['teamname'] == team2].index
            
            # Update team1
            all_teams.loc[team1_idx, 'playable'] = 1
            all_teams.loc[team1_idx, 'odds'] = game['odds_team1']
            all_teams.loc[team1_idx, 'match no'] = match_counter
            
            # Update team2
            all_teams.loc[team2_idx, 'playable'] = 1
            all_teams.loc[team2_idx, 'odds'] = 1 - game['odds_team1']  # Team2 odds
            all_teams.loc[team2_idx, 'match no'] = match_counter
            
            match_counter += 1
        
        all_day_dfs.append(all_teams)
